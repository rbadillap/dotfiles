---
title: Accounts
description: Log in to account-based CLIs without leaving a token on disk, and what each login costs.
---
...
|--------------------------|-------------------|--------------------|
| [GitHub](./github.mdx)      | not measured      | ~10 min once (2FA, fine-grained token); per Mac: not measured |
| [Vercel](./vercel.mdx)      | not measured      | a few minutes once (token); per Mac: under a minute |
| [AWS](./aws.mdx)            | one browser approval per SSO session (8 h by default); no keys | not used: SSO has no long-lived secret |
