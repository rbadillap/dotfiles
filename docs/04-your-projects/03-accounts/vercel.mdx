...
3. It finishes by printing a command that adds a line to `~/.zshrc`. Skip it:
   `config/shell/op.zsh` already loads the plugins ([Shell and terminal](../../03-your-mac/04-shell-and-terminal.mdx)).
...
[Varlock](../02-secrets/index.mdx), `.env.schema` issues one on each run:
