...
file_managed_block() {
  ...
  # First the markers, before anything is decided: replacing runs from the
  # begin marker to the end marker, so a missing, stray or repeated marker
  # would swallow lines outside the block. Zero blocks or exactly one, closed.
  if [ -f "$1" ] && ! FB_BEGIN=$begin FB_END=$end awk '
    index($0, ENVIRON["FB_BEGIN"]) == 1 { if (seen) bad = 1; seen = open = 1; next }
    $0 == ENVIRON["FB_END"] { if (!open) bad = 1; open = 0 }
    END { exit bad || open }
  ' "$1"; then
    fail "$(printf %s "$1" | sed "s#^$HOME#~#"): its \"$begin\" block is unclosed, repeated or has a stray end marker; fix it by hand"
    return 0
  fi
  if [ -f "$1" ] && FB_CONTENT=$3 awk 'BEGIN { RS = "\001"; c = ENVIRON["FB_CONTENT"] } index($0, c) { f = 1 } END { exit !f }' "$1"; then
    return 0
